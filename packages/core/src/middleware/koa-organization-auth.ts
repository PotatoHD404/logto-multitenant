import { buildOrganizationUrn, getOrganizationIdFromUrn } from '@logto/core-kit';
 ...
 */
export default function koaOrganizationAuth<StateT, ContextT extends IRouterParamContext, ResponseBodyT>(
  queries: Queries
  ...
  return async (ctx, next) => {
    ...
    debugConsole.info(`Validating organization membership for user ${userId} in organization ${organizationId}`);
    ...
    try {
      ...
      debugConsole.info(`Organization membership validated for user ${userId} in organization ${organizationId}`);
...
function getOrganizationIdFromRequest(request: Request, auth: any): string | undefined {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    try {
      ...
      if (parts.length === 3 && parts[1]) {
        ...
        if (payload.aud && typeof payload.aud === 'string' && payload.aud.startsWith('urn:logto:organization:')) {
  ...
  if (request.url?.includes('/organizations/')) {
    const match = request.url.match(/\/organizations\/([^\/\?]+)/);
    if (match?.[1]) {
      return match[1];
    }
  }
